...
namespace CityGen
{
    ...
    public class LoftRoadBehaviour : MonoBehaviour
    {
        ...
        public List<SplineData<float>> Widths
        {
            get
            {
                foreach (var width in floatData)
                {
                    if (width.DefaultValue == 0)
                        width.DefaultValue = m_Width;
                }
        ...
        void OnSplineContainerAdded(SplineContainer container, int index)
        {
            ...
            if (floatData.Count < LoftSplines.Count)
            {
                var delta = LoftSplines.Count - floatData.Count;
                for (var i = 0; i < delta; i++)
                {
#if  UNITY_EDITOR
                    Undo.RecordObject(this, "Modifying Widths SplineData");
#endif
                    floatData.Add(new SplineData<float>() { DefaultValue = 1f });
                }
            }
        ...
        public void Loft(Spline spline)
        {
